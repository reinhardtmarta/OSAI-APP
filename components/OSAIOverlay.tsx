...
import { isExplicitConsent, isExplicitDenial } from '../services/policy';
...
export const OSAIOverlay: React.FC<OSAIOverlayProps> = ({ 
  ...
  isTtsEnabled, isAccessibilityMode, onApprove, onDeny, onAnalyze, onUpdatePolicy
}) => {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-[999] pointer-events-none" style={{ transform: `scale(${ui.scale})`, opacity: ui.transparency }}>
      <div className={`pointer-events-auto glass rounded-[44px] border-2 transition-all duration-500 shadow-2xl flex flex-col ${isMinimized ? 'w-20 h-20' : 'w-85 h-[620px] max-h-[85vh]'} ${
        ...
        <div className="p-5 flex items-center justify-between border-b border-white/5 shrink-0 bg-white/5 rounded-t-[44px]">
          <div className="flex items-center gap-3">
            <div className={`p-2.5 rounded-full transition-all duration-300 relative ${!anyMicActive ? 'bg-slate-700' : isAwake ? 'bg-blue-600 shadow-[0_0_15px_rgba(37,99,235,0.5)]' : 'bg-slate-800'}`}>
          ...
          <div className="flex items-center gap-1">
            {!isMinimized && (
              <button onClick={clearHistory} className="p-2.5 hover:bg-white/10 rounded-xl text-white/30 transition-colors">
                <Trash2 size={16} />
              </button>
            )}
            <button onClick={() => { setIsMinimized(!isMinimized); Haptics.light(); }} className="p-2.5 hover:bg-white/10 rounded-xl text-white/30 transition-colors">
        ...
        {!isMinimized && (
          <>
            <div ref={scrollContainerRef} className="flex-1 overflow-y-auto p-5 space-y-6 bg-black/40 custom-scrollbar flex flex-col relative pb-10">
              <div className="flex-1 space-y-5">
                {history.map(m => (
                  <ChatBubble key={m.id} role={m.role} text={m.text} steps={m.steps} riskLevel={m.riskLevel} />
                ))}
              </div>
            ...
            <div className="p-6 border-t border-white/10 bg-black/80 backdrop-blur-3xl shrink-0 rounded-b-[44px]">
              <div className="flex gap-3 items-center">
                 <div className="relative flex-1">
                    ...
                    />
                    <button onClick={handleManualSubmit} className="absolute right-4 top-1/2 -translate-y-1/2 text-blue-400 p-2 hover:scale-110 active:scale-95 transition-all">
                 ...
                 <div className="flex gap-2.5">
                    ...
                    <div className="w-15 h-15 rounded-[30px] flex items-center justify-center border-2 border-white/10 bg-white/5 shadow-lg">
